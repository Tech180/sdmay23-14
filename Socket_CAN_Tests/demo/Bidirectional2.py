...
def finalize_send_message_vcan1():
  ...
  if random.randint(1,5) == random.randint(1,5):
    hexArray = [int(first4Bytes[2:4], 16), int(first4Bytes[2:4], 16), int(first4Bytes[4:6], 16), int(first4Bytes[6:8], 16)]
  else: 
...
def update_fv_list(fv):
    ...
    if len(last_64_freshness_values) == 63:
        ...
        last_64_freshness_values[idx_of_next_fv]=int(fv,16)
...
def validate_counter(msg):
    ...
    if int(received_fv, 16) in last_64_freshness_values:
        return False
    if int(received_fv, 16) < int(smallest_in_FV_list(), 16):
       return False
    update_fv_list(received_fv)
    return True
